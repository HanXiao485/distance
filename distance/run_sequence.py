...
# libgomp requires positive integer thread counts. Some managed runtimes set
# these variables to "0", which emits a warning as soon as NumPy/PyTorch is
# imported and can break native-library initialization.
for _thread_env in ("OMP_NUM_THREADS", "MKL_NUM_THREADS"):
    try:
        _thread_count = int(os.environ.get(_thread_env, "1"))
    except ValueError:
        _thread_count = 0
    if _thread_count < 1:
...
def discover_frames(seq_cfg: dict[str, Any], require_pred_mask: bool = True) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    if pred_mask_dir.exists():
        for _mask_ext in ("*.png", "*.jpg", "*.jpeg"):
            for p in pred_mask_dir.glob(_mask_ext):
                try:
...
def main() -> None:
    ...
    if not frames:
        print("[WARN] No processable frames were discovered.")
        if discovery_meta:
            for item in discovery_meta.get("skipped_detail", [])[:10]:
                print(f"  - {item['frame_id']}: {', '.join(item['missing'])}")
        return
    ...
    if seg_cfg.get("enabled", False):
        ...
        pred_mask_map  = run_inference(
            ...
            class_ids=seg_cfg.get("class_ids"),
        )
