...
GOOSE 支持包含的 4 个文件（已在镜像内 `/workspace/WildScenes` 树中）：
...
| **数据输入卷挂载** | 容器内路径 `/root/distance/WildScenes/DISTANCE/datasets/` | 宿主机侧指向平台上 GOOSE 数据目录 |
| **输出卷挂载** | 容器内路径 `/workspace/WildScenes/work_dirs` | checkpoint / 日志 / 最优权重持久保存 |
| **环境变量** | `PYTHONUNBUFFERED=1` | 非必需；让日志实时输出。无强制项可留空 |
| **配置文件** | 无 | 配置已烤进镜像，命令行直接引用 |

> 注意：代码已烤进镜像，**不要再 bind-mount 整个 `/workspace/WildScenes`**，否则会盖掉镜像内的 GOOSE 代码。只挂 data 和 work_dirs。
...
```bash
cd /workspace/WildScenes

# 自动定位 goose_2d_train 的上一层作为 goose_root（无论挂载后是哪一层都能找到）
GROOT=$(dirname "$(find /root/distance/WildScenes/DISTANCE/datasets -maxdepth 3 -type d -name goose_2d_train | head -1)")
echo "goose_root = $GROOT"

python scripts/data/setup_goose.py \
  --goose_root "$GROOT" \
  --save_path  /root/distance/WildScenes/DISTANCE/datasets/processed/goose2d
```
...
> `--save_path` 必须是 `/root/distance/WildScenes/DISTANCE/datasets/processed/goose2d`，因为数据集配置里的 `data_root` 就指向这里。
...
torchrun --nproc_per_node=2 \
  ...
  --launcher pytorch \
  --work-dir /workspace/WildScenes/work_dirs/deeplabv3_goose_2x4090 \
...
python scripts/benchmark/eval2d.py \
  wildscenes/configs/deeplabv3/deeplabv3_r50-d8_2xb20-80k_goose-512x512.py \
  /workspace/WildScenes/work_dirs/deeplabv3_goose_2x4090/best_mIoU_iter_*.pth \
  --launcher none \
  --work-dir /workspace/WildScenes/vis_eval/deeplabv3_goose \
  --show-dir painted
```
...
| 找不到 `goose_2d_train` | 数据挂载层级异常 | 手动 `ls /root/distance/WildScenes/DISTANCE/datasets/` 确认结构，把含 `goose_2d_train/val` 的目录传给 `--goose_root` |
